...
export async function GET() {
  ...
  try {
    ...
    const result = await pool.request().input('id', sql.UniqueIdentifier, dbUserId).query('SELECT TOP 1 u.email,u.profile_completed,p.full_name,p.birth_date,p.gender,p.phone,p.school_class,p.emergency_contact FROM users u JOIN profiles p ON p.user_id=u.id WHERE u.id=@id')
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    try {
      await new sql.Request(transaction).input('id', sql.UniqueIdentifier, dbUserId).input('name', sql.NVarChar(255), body.fullName.trim()).input('phone', sql.NVarChar(30), body.phone?.trim() || null).input('birthDate', sql.Date, body.birthDate || null).input('gender', sql.NVarChar(30), body.gender?.trim() || null).input('schoolClass', sql.NVarChar(50), body.schoolClass?.trim() || null).input('emergency', sql.NVarChar(sql.MAX), body.emergencyContact ? JSON.stringify(body.emergencyContact) : null).query('UPDATE profiles SET full_name=@name,phone=@phone,birth_date=@birthDate,gender=@gender,school_class=@schoolClass,emergency_contact=@emergency WHERE user_id=@id')
      await new sql.Request(transaction).input('id', sql.UniqueIdentifier, dbUserId).query('UPDATE users SET profile_completed=1,updated_at=SYSUTCDATETIME() WHERE id=@id')
