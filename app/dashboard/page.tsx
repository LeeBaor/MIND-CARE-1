...
import { Bell, BookOpenCheck, Brain, CalendarDays, Check, ChevronRight, ClipboardCheck, CreditCard, HeartPulse, LockKeyhole, Moon, ShieldCheck, UsersRound } from 'lucide-react'
import { SiteHeader } from '@/components/site-header'
import { SiteFooter } from '@/components/site-footer'
import { BachMaiNav } from '@/components/bachmai-nav'
import { SosButton } from '@/components/sos-button'
import { getHomework, getMood, saveHomework, saveMood, type MindBooking } from '@/lib/mind-care-store'
...
export default function DashboardPage() {
  ...
  useEffect(() => { setMood(getMood()); setHomework(getHomework()); fetch('/api/bookings').then(response => response.ok ? response.json() : Promise.reject()).then(setBookings).catch(() => setBookings([])) }, [])
